window.onload = function () {
    ...
    comment.addEventListener('keypress', function (e) {
        if (e.key === 'Enter') {
            if (e.target.value) {
                ...
                alert('⚠️Please Enter A Valid Comment⚠️')
            }
        }
    })
        ...
        if (commentHolder.hasChildNodes(e.target)) {
            if (e.key === 'Enter') {
                ...
                if (value) {
                    ...
                    alert('⚠️Please Enter A Valid Reply⚠️')
                }
            }
        }
    })
    ...
    likeBtn.addEventListener('click', function (e) {
        ...
        reqLikeDislike('likes', postId)
            .then(res => res.json())
            .then(data => {
                let likeText = data.liked ? 'Liked It' : 'Like It'
                likeText = likeText + ` ( ${data.totalLikes} )`
                let dislikeText = `Love It ( ${data.totalDislikes} )`
    ...
    dislikeBtn.addEventListener('click', function (e) {
        ...
        reqLikeDislike('dislikes', postId)
            .then(res => res.json())
            .then(data => {
                let dislikeText = data.disliked ? 'Loved It' : 'Love It'
                dislikeText = dislikeText + ` ( ${data.totalDislikes} )`
                let likeText = `Like It ( ${data.totalLikes} )`
